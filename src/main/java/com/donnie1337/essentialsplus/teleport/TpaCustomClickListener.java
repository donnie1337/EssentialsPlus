...
public final class TpaCustomClickListener implements Listener {
    ...
    public TpaCustomClickListener(Plugin plugin, TeleportService teleportService) { this.plugin = plugin; this.teleportService = teleportService; }
    ...
    private void handleCustomClick(Event event) {
        try {
            ...
            if (timeout > 0 && System.currentTimeMillis() - action.createdAt() >= timeout) {
                ...
                results.put(token, expiredResult); sendResultMessage(player, expiredResult, action); return;
            }
            boolean handled = teleportService.handleButton(player, token);
            if (!handled) return;
            ButtonResult result = switch (action.type()) { case ACCEPT -> ButtonResult.ACCEPTED; case DENY -> ButtonResult.DENIED; case CANCEL -> ButtonResult.CANCELLED; };
    ...
    private enum ButtonResult { ACCEPTED, DENIED, EXPIRED, CANCELLED, CANCEL_BLOCKED_ACCEPTED, CANCEL_BLOCKED_DENIED, CANCEL_BLOCKED_EXPIRED }
}
